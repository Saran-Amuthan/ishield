...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $required_fields = array("firstName", "lastName", "gender", "address1", "city", "state", "countryOfResidence", "zip", "mobile", "dateOfBirth", "idType", "idNumber", "termsAgree");
    $error = false;
    ...
    // Exit script if there are validation errors
    if ($error) {
        echo "<a href='javascript:history.back()'>Go back and fill in missing fields.</a>";
        exit();
    }
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["idFront"]["tmp_name"], $idFrontFile) && move_uploaded_file($_FILES["idBack"]["tmp_name"], $idBackFile)) {
            ...
            $stmt->bind_param("ssssssssssssss", $firstName, $lastName, $gender, $address1, $address2, $city, $state, $country, $zip, $mobile, $dateOfBirth, $idType, $idNumber, $termsAgree);

            if ($stmt->execute()) {
                // Redirect to user_kyc.html with parameters
                header("Location: kyc.html?firstName=$firstName&lastName=$lastName&gender=$gender&address1=$address1&address2=$address2&city=$city&state=$state&country=$country&zip=$zip&mobile=$mobile&dateOfBirth=$dateOfBirth&idType=$idType&idNumber=$idNumber&termsAgree=$termsAgree");
                exit();
            } else {
